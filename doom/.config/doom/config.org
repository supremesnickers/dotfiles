...
(defun uni-choose-course-symlink (&optional path)
  ...
  (interactive "P")
  (unless path
    (setq path (ivy-read "Select course: " (uni-get-all-courses))))
  (if (-contains? (uni-get-all-courses) path)
      (uni-set-course-symlink path)
    (when (y-or-n-p
...
(defun uni-init-course (path)
  "Creates the course directory structure from an info.yml file."
